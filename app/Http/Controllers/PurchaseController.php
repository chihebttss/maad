...
class PurchaseController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach ($product_id as $i => $id) {
            ...
            if($lims_product_data->cost != $new_cost ){
                $total_qty_purchases = DB::table('purchases')
                    ->join('product_purchases', 'purchases.id', '=', 'product_purchases.purchase_id')->where([
                        ['product_purchases.product_id', $id],
                        ['purchases.warehouse_id', $data['warehouse_id']]
                    ])
                    ->sum('qty');
                $total_amount_purchases = DB::table('purchases')
                    ->join('product_purchases', 'purchases.id', '=', 'product_purchases.purchase_id')->where([
                        ['product_purchases.product_id', $id],
                        ['purchases.warehouse_id', $data['warehouse_id']]
                    ])->sum('total');
                $lims_product_data->cost=round($total_amount_purchases/$total_qty_purchases, 0);
            }
            $lims_product_data->save();
