...
class StockCountController extends Controller
{
    ...
    public function stockdispo(Request $request){
        ...
        if( $role->hasPermissionTo('stock_count') ) {
            ...
            $q = Product_Warehouse::all();
            return view('stock_dispo.index', compact('lims_warehouse_list','q','warehouse_id'));
        }
        else
    ...
    public function stockData(Request $request){
        ...
        if(!empty($stocks))
        {
            ...
            {
                $nestedData['id'] = $stock->id;
                $nestedData['key'] = $key;

                $produit=Product::find($stock->product_id);
                 $entrepot=Warehouse::find($stock->warehouse_id);
                $nestedData['produit'] = $produit->name;
                $nestedData['entrepot'] = $entrepot->name;


                $nestedData['stock_qty'] = $stock->qty;
